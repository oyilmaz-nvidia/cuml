...
template<typename T>
class CdParalTest: public ::testing::TestWithParam<CdParalInputs<T> > {
protected:
	void test() {
		params = ::testing::TestWithParam<CdParalInputs<T>>::GetParam();
...
const std::vector<CdParalInputs<float> > inputsf2 = { { 0.01f, 4, 2 } };

const std::vector<CdParalInputs<double> > inputsd2 = { { 0.01, 4, 2 } };

typedef CdParalTest<float> CdParalTestF;
TEST_P(CdParalTestF, Fit) {
...
typedef CdParalTest<double> CdParalTestD;
TEST_P(CdParalTestD, Fit) {
...
INSTANTIATE_TEST_CASE_P(CdParalTests, CdParalTestF, ::testing::ValuesIn(inputsf2));

INSTANTIATE_TEST_CASE_P(CdParalTests, CdParalTestD, ::testing::ValuesIn(inputsd2));
