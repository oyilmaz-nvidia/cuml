...
cdef extern from "solver/solver_c.h" namespace "ML::Solver":
    ...
    cdef void cdParalFit(float *input,
		   int n_rows,
		   int n_cols,
		   float *labels,
		   float *coef,
    ...
    cdef void cdParalFit(double *input,
		   int n_rows,
		   int n_cols,
		   double *labels,
		   double *coef,
...
class ParallelCD:
    ...
    def __init__(self, loss='squared_loss', alpha=0.0001, l1_ratio=0.15, 
        fit_intercept=True, normalize=False, max_iter=1000, tol=1e-3, n_processors=4):
    ...
    def fit(self, X, y):
        ...
        if self.gdf_datatype.type == np.float32:
            cdParalFit(<float*>X_ptr, 
                       ...
                       <float>self.l1_ratio,
                       <float>self.tol,
                       <int>self.n_processors)

            self.intercept_ = c_intercept1
        else:
            cdParalFit(<double*>X_ptr, 
                       <int>self.n_rows, 
                       <int>self.n_cols, 
                       <double*>y_ptr, 
                       <double*>coef_ptr,
